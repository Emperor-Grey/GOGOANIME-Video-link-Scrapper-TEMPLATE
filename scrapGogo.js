...
const fetchDetails = async (link) => {
    try {
        ...
        const description = $('#wrapper_bg > section > section.content_left > div.main_body > div:nth-child(2) > div.anime_info_body_bg > p:nth-child(5)')
            .text()
            .substring(14);
        ...
        // Extract genres
        $('#wrapper_bg > section > section.content_left > div.main_body > div:nth-child(2) > div.anime_info_body_bg > p:nth-child(6) > a').each((i, element) => {
            const genre = $(element).text();
            genres.push(genre);
        });

        return {
            title, image, type, description, genres,
        };
    } catch (error) {
