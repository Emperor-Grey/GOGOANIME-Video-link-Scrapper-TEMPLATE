...
// Define a class named VideoExtractor
    ...
    async extract(videoUrl) {
        ...
        if (decryptedData.source[0].file.includes('.m3u8')) {
            ...
            decryptedData.source.forEach((source) => {
                this.addSources({
                    url: source.file, isM3U8: source.file.includes('.m3u8'), quality: 'default',
                });
            });
        } else {
            ...
            decryptedData.source.forEach((source) => {
                this.addSources({
                    url: source.file, isM3U8: source.file.includes('.m3u8'), quality: source.label.split(' ')[0] + 'p',
                });
            });
        }
        ...
        decryptedData.source_bk.forEach((source) => {
            this.addSources({
                url: source.file, isM3U8: source.file.includes('.m3u8'), quality: 'backup',
            });
        });
    ...
    async addSources(source) {
        ...
        if (source.url.includes('m3u8')) {
            ...
            for (const video of videoList || []) {
                ...
                this.sources.push({
                    url: url, quality: `${quality}p`, isM3U8: true,
                });
            }
        ...
        this.sources.push({
            url: source.file, isM3U8: source.file.includes('.m3u8'),
        });
    }
